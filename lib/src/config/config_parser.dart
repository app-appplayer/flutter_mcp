...
class ConfigParser {
  ...
  String getString(String key, {
    ...
    if (value == null) {
      if (required) {
        throw MCPConfigurationException('Required string field "$key" is missing from $_configName');
      }
      return defaultValue ?? '';
    }
  ...
  bool getBool(String key, {
    ...
    if (value == null) {
      if (required) {
        throw MCPConfigurationException('Required boolean field "$key" is missing from $_configName');
      }
  ...
  int getInt(String key, {
    ...
    if (value == null) {
      if (required) {
        throw MCPConfigurationException('Required integer field "$key" is missing from $_configName');
      }
  ...
  double getDouble(String key, {
    ...
    if (value == null) {
      if (required) {
        throw MCPConfigurationException('Required double field "$key" is missing from $_configName');
      }
  ...
  List<T> getList<T>(String key, T Function(dynamic) itemParser, {
    ...
    if (value == null) {
      if (required) {
        throw MCPConfigurationException('Required list field "$key" is missing from $_configName');
      }
      return defaultValue ?? [];
    }
  ...
  /// Parse map value
  Map<String, T> getMap<T>(String key, T Function(dynamic) valueParser, {
    ...
    if (value == null) {
      if (required) {
        throw MCPConfigurationException('Required map field "$key" is missing from $_configName');
      }
      return defaultValue ?? {};
    }
  ...
  ConfigParser getObject(String key, {
    bool required = false,
  }) {
    final value = _json[key];
    ...
    if (value == null) {
      if (required) {
        throw MCPConfigurationException('Required object field "$key" is missing from $_configName');
      }
  ...
  void validateRequired(List<String> requiredFields) {
    final missing = requiredFields.where((field) => !_json.containsKey(field)).toList();
  ...
  void validateNoUnknownFields(List<String> allowedFields) {
    final unknown = _json.keys.where((field) => !allowedFields.contains(field)).toList();
  ...
  @override
  String toString() => 'ConfigParser($_configName: ${_json.keys.length} fields)';
}
...
class ConfigPatterns {
  ...
  static Duration parseTimeout(ConfigParser parser, String key, {
  ...
  static ({int maxRetries, Duration delay}) parseRetryConfig(ConfigParser parser, String prefix) {
    return (
  ...
  static String parseUrl(ConfigParser parser, String key, {
    String? defaultValue,
    bool required = false,
  }) {
    final url = parser.getString(key, defaultValue: defaultValue, required: required);
  ...
  static String parseFilePath(ConfigParser parser, String key, {
    String? defaultValue,
    bool required = false,
  }) {
    final path = parser.getString(key, defaultValue: defaultValue, required: required);
    
    if (path.isNotEmpty && !ConfigValidator.isValidFilePath(path)) {
      throw MCPConfigurationException('Invalid file path format for "$key": $path');
    }
