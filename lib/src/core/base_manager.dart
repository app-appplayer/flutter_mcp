...
abstract class BaseManager with OperationWrapperMixin, HealthCheckMixin {
  ...
  BaseManager(this.managerName) : logger = Logger('flutter_mcp.$managerName');
  ...
  Map<String, dynamic> getStatus() {
    return {
      ...
      'health': HealthMonitor.instance.currentHealth['components']
              ?[componentId] ??
          'unknown',
    };
  }
...
abstract class ConfigBasedManager<T> extends BaseManager {
  ...
  ConfigBasedManager(super.managerName);
  ...
  Future<void> onInitialize() async {
    ...
    if (loadedConfig != null) {
      ...
      throw MCPConfigurationException(
          'Failed to load configuration for $managerName');
    }
  }
  ...
  Future<void> onInitialize() async {
    ...
    if (!isPlatformSupported()) {
      throw MCPPlatformNotSupportedException(
        ...
        resolution:
            'Use a different platform or check platform-specific requirements',
      );
    }
...
abstract class ResourceManagingManager extends BaseManager {
  ...
  ResourceManagingManager(super.managerName);
