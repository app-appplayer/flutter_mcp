...
class EnhancedBatchConfig {
  ...
  }) : retryDelay = retryDelay ?? ((attempt) => Duration(milliseconds: 100 * math.pow(2, attempt).toInt()));
...
class EnhancedBatchProcessor<T> implements HealthCheckProvider {
  ...
  final Map<BatchRequestPriority, Queue<EnhancedBatchRequest<T>>> _requestQueues = {
  ...
  EnhancedBatchProcessor({
    ...
    _circuitBreaker = CircuitBreaker(
      ...
      onOpen: () => this.logger.warning('Circuit breaker opened for batch processor: $processorId'),
      onClose: () => this.logger.info('Circuit breaker closed for batch processor: $processorId'),
    );
  }
  ...
  EnhancedBatchRequest<T>? _getOldestRequest() {
    ...
    for (final queue in _requestQueues.values) {
      if (queue.isNotEmpty && (oldest == null || queue.first.createdAt.isBefore(oldest.createdAt))) {
        oldest = queue.first;
      }
    }
    return oldest;
  ...
  void _scheduleNextBatch() {
    ...
    _processTimer = Timer(config.maxWaitTime, () {
      _processTimer = null;
      if (_getTotalPendingRequests() >= config.minBatchSize || _hasTimedOutRequests()) {
        _processBatch();
      } else {
        _scheduleNextBatch();
      }
  ...
  List<EnhancedBatchRequest<T>> _collectBatch() {
    ...
    if (config.deduplicateRequests) {
      _deduplicationCache.removeWhere((key, request) => 
        request.age > Duration(seconds: 10)
      );
    }
  ...
  Future<MCPHealthCheckResult> performHealthCheck() async {
    ...
    if (!_isRunning) {
      ...
      status = MCPHealthStatus.degraded;
      message = 'Low success rate: ${(statistics.successRate * 100).toStringAsFixed(1)}%';
    } else {
...
class EnhancedBatchManager implements HealthCheckProvider {
  ...
  Future<void> _processBatchInternal<T>(
    ...
    stats.averageWaitTime = Duration(
      milliseconds: waitTimes.map((d) => d.inMilliseconds).reduce((a, b) => a + b) ~/ waitTimes.length
    );
    ...
    final futures = requests.map((req) async {
      ...
      try {
        final result = await req.requestFunction().timeout(config.requestTimeout);
    ...
    if (avgExecTime > 0) {
      stats.averageExecutionTime = Duration(
        milliseconds: avgExecTime ~/ requests.where((r) => r.lastExecutionTime != null).length
      );
    }
  ...
  void _handleRequestError<T>(
    ...
    if (config.retryFailedRequests && request.retryCount < config.maxRetries) {
      ...
      _logger.fine('Retrying request ${request.id} (attempt ${request.retryCount})');
    } else {
      stats.failedRequests++;
      request.completer.completeError(error, stackTrace);
      
      _logger.warning('Request ${request.id} failed after ${request.retryCount} retries');
    }
  }
  
  /// Publish batch processing metrics
  void _publishBatchMetrics(String llmId, int batchSize, Duration executionTime) {
    _eventSystem.publish('batch.processed', {
      'llmId': llmId,
      'batchSize': batchSize,
      ...
      'statistics': {
        ...
        'averageExecutionTimeMs': _statistics[llmId]!.averageExecutionTime.inMilliseconds,
      },
    });
  }
  ...
  Future<MCPHealthCheckResult> performHealthCheck() async {
    ...
    final processorHealths = await Future.wait(
      _processors.entries.map((e) async => {
        'llmId': e.key,
