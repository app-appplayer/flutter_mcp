...
class DependencyNode {
  ...
  @override
  String toString() => 'DependencyNode($serviceKey, level: $level, deps: ${dependencies.length})';
}
...
class EnhancedDIContainer {
  ...
  void register<T>({
    ...
    final registration = ServiceRegistration<T>(
      ...
      isSingleton: isSingleton,
      dependencies: dependencies,
      onInitialize: onInitialize != null ? (dynamic instance) => onInitialize(instance as T) : null,
      onDispose: onDispose != null ? (dynamic instance) => onDispose(instance as T) : null,
  ...
  Map<String, dynamic> getStatistics() {
    ...
    final singletonCount = _registrations.values.where((r) => r.isSingleton).length;
    ...
    return {
      ...
      'circularDependencies': _resolveDependencies().circularDependencies.length,
    };
  }
  ...
  Future<void> _initializeService(ServiceRegistration registration) async {
    ...
    if (_initializing.contains(key)) {
      throw MCPException('Circular dependency detected while initializing: $key');
    }
  ...
  Future<void> _initializeDependencies(ServiceRegistration registration) async {
    for (final dependencyType in registration.dependencies) {
      ...
      if (dependencyRegistration != null && !_initialized.contains(dependencyKey)) {
  ...
  void _buildDependencyGraph(ServiceRegistration registration) {
    ...
    final node = _dependencyGraph.putIfAbsent(serviceKey, () => DependencyNode(serviceKey));
    ...
    for (final dependencyType in registration.dependencies) {
      ...
      if (existingDepNode != null && existingDepNode.dependencies.contains(serviceKey)) {
        throw MCPException('Circular dependency detected: $serviceKey <-> $dependencyKey');
      }
      
      // Create dependency node if it doesn't exist
      final depNode = _dependencyGraph.putIfAbsent(dependencyKey, () => DependencyNode(dependencyKey));
  ...
  void _checkForCircularDependencies(String serviceKey) {
    ...
    void dfs(String currentKey) {
      if (inStack.contains(currentKey)) {
        throw MCPException('Circular dependency detected involving: $currentKey');
      }
  ...
  void _dfsTopologicalSort(String serviceKey, List<String> order, List<String> circularDeps) {
  ...
  int _calculateServiceLevel(String serviceKey, Map<String, int> levels, Set<String> visiting) {
  ...
  void _publishLifecycleEvent(
    ...
    _logger.fine('Publishing lifecycle event: $serviceKey $oldState -> $newState');
...
mixin EnhancedDIAware {
  @protected
  Future<T> inject<T>({String? name}) => EnhancedServiceLocator.get<T>(name: name);
  
  @protected
  bool canInject<T>({String? name}) => EnhancedServiceLocator.isRegistered<T>(name: name);
  
  @protected
  bool isInjected<T>({String? name}) => EnhancedServiceLocator.isInitialized<T>(name: name);
}