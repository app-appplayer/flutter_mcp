...
class EventHandler<T extends McpEvent, R> {
  ...
  bool shouldHandle(T event) {
    ...
    if (timeout != null && _lastInvocation != null) {
      final timeSinceLastInvocation = DateTime.now().difference(_lastInvocation!);
...
class EnhancedTypedEventSystem {
  ...
  // Singleton instance
  static final EnhancedTypedEventSystem _instance = EnhancedTypedEventSystem._internal();
  ...
  Future<void> publish<T extends McpEvent>(T event) async {
    ...
    try {
      ...
      if (_isPaused) {
        ...
        _logger.fine('Event queued: ${processedEvent.eventType} (system paused)');
        return;
      }
      
      await _publishImmediately(processedEvent!);
  ...
  String subscribeAdvanced<T extends McpEvent, R>({
    ...
    _handlers[eventType]!.sort((a, b) => (b.priority ?? 0).compareTo(a.priority ?? 0));
    ...
    final streamSubscription = _eventControllers[eventType]!.stream
    ...
    _logger.fine('Added advanced subscription for $eventType with ID: $handlerId');
  ...
  String subscribePattern({
    ...
    _subscriptions[subscriptionId] = EventSubscription<McpEvent>(
      ...
      handler: handler,
      description: description,
      metadata: {...metadata, 'type': 'pattern', 'subSubscriptions': subscriptions},
  ...
  void addMiddleware(EventMiddleware middleware) {
    ...
    _logger.info('Added middleware: ${middleware.name} (priority: ${middleware.priority})');
  }
  ...
  Future<void> resume() async {
    ...
    _logger.fine('Enhanced event system resumed, processed $queuedCount queued events');
  }
