...
class TypedEventSystem {
  ...
  String subscribe<T extends McpEvent>(void Function(T) callback) {
    ...
    _subscriptions[token] = _eventControllers[type]!.stream
  ...
  String subscribeMultiple<T extends McpEvent>(
    ...
    for (final type in eventTypes) {
      ...
      final subscription = _eventControllers[type]!.stream
          .where((event) => event is T)
          .cast<T>()
          .listen(callback);
    ...
    _logger.fine('Added multi-type subscription for ${eventTypes.length} types with token: $token');
