...
class EnhancedNotificationConfig {
  ...
  Map<String, dynamic> toMap() {
    ...
    if (groupAlertBehavior != null) {
      map['groupAlertBehavior'] = groupAlertBehavior;
    }
    ...
    if (messages != null) {
      map['messages'] = messages!.map((m) => m.toMap()).toList();
    }
    if (actions != null) {
      map['actions'] = actions!.map((a) => a.toMap()).toList();
    }
    ...
    if (mediaControls != null) {
      map['mediaControls'] = mediaControls!.map((c) => c.toMap()).toList();
    }
...
class EnhancedNotificationManager {
  ...
  // Action handlers
  final Map<String,
          void Function(String notificationId, Map<String, dynamic>? data)>
      _actionHandlers = {};

  // Reply handlers
  final Map<String, void Function(String notificationId, String reply)>
      _replyHandlers = {};
  ...
  Future<void> initialize({NotificationManager? baseManager}) async {
    ...
    if (_baseManager is AndroidNotificationManager) {
      (_baseManager as AndroidNotificationManager)
          .registerClickHandler('default', _handleNotificationClick);
    } else if (_baseManager is IOSNotificationManager) {
      (_baseManager as IOSNotificationManager)
          .registerClickHandler('default', _handleNotificationClick);
    } else if (_baseManager is DesktopNotificationManager) {
      (_baseManager as DesktopNotificationManager)
          .registerClickHandler('default', _handleNotificationClick);
    }
  ...
  Future<void> updateProgress(
    ...
    if (config == null || config.style != NotificationStyle.progress) {
      _logger
          .warning('Cannot update progress for non-progress notification: $id');
      return;
    }
  ...
  Future<void> _showBasicNotification(EnhancedNotificationConfig config) async {
    if (_baseManager is AndroidNotificationManager) {
      await (_baseManager as AndroidNotificationManager).showNotification(
        ...
        data: _buildNotificationData(config),
      );
    } else {
  ...
  Future<void> _showBigTextNotification(
      EnhancedNotificationConfig config) async {
  ...
  Future<void> _showMediaNotification(EnhancedNotificationConfig config) async {
    ...
    if (config.mediaControls != null) {
      data['mediaControls'] =
          config.mediaControls!.map((c) => c.toMap()).toList();
    }
  ...
  Future<void> _showProgressNotification(
    ...
    if (config.indeterminate != null) {
      data['indeterminate'] = config.indeterminate;
    }
  ...
  Future<void> _showGroupSummary(String groupKey) async {
    ...
    for (final id in notifications.take(5)) {
      // Show up to 5 items
      final config = _activeNotifications[id];
      if (config != null) {
  ...
  Map<String, dynamic> _buildNotificationData(
      EnhancedNotificationConfig config) {
    ...
    if (config.enableReply) {
      ...
      if (config.replyPlaceholder != null) {
        data['replyPlaceholder'] = config.replyPlaceholder;
      }
    }
