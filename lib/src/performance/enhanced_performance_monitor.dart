...
class AnomalyDetector {
  ...
  void _calculateStats() {
    ...
    final variance =
        _window.map((x) => math.pow(x - _mean!, 2)).reduce((a, b) => a + b) /
            _window.length;
...
class MetricAggregator {
  ...
  Map<String, dynamic> getStatistics() {
    ...
    final variance =
        values.map((x) => math.pow(x - avg, 2)).reduce((a, b) => a + b) /
            values.length;
...
class EnhancedPerformanceMonitor {
  ...
  void enableAutoDetection({
    ...
    _logger.info(
        'Auto-detection enabled (anomalies: $anomalies, thresholds: $thresholds)');
  }
  ...
  Map<String, dynamic>? getMetricStatistics(String metricName) {
    ...
    // Check typed metrics
    if (report['typed_metrics'] != null &&
        report['typed_metrics'].containsKey(metricName)) {
    ...
    if (summary['counters'] != null &&
        summary['counters'].containsKey(metricName)) {
      return {
    ...
    // Check timers
    if (summary['timers'] != null &&
        summary['timers'].containsKey(metricName)) {
  ...
  void _checkAnomaly(PerformanceMetric metric) {
    ...
    if (detector.isAnomaly(metric.value)) {
      _logger.warning(
          'Anomaly detected in metric ${metric.name}: ${metric.value}');
  ...
  void _checkThreshold(PerformanceMetric metric) {
    ...
    } else if (config.warningLevel != null &&
        metric.value >= config.warningLevel!) {
      violation = ThresholdViolation(
        metricName: metric.name,
        value: metric.value,
    ...
    if (violation != null) {
      ...
      if (config.sustainedDuration != null) {
        final sustained =
            tracker.getSustainedViolation(config.sustainedDuration!);
        if (sustained != null) {
          config.onViolation?.call(sustained);

          _logger.warning('Sustained threshold violation for ${metric.name}: '
    ...
    for (final entry in _thresholdTrackers.entries) {
      if (entry.value.hasViolations) {
        violations[entry.key] = {
          ...
          'lastViolation':
              entry.value.lastViolation?.timestamp.toIso8601String(),
        };
      }
    }
...
class _ThresholdTracker {
  ...
  ThresholdViolation? getSustainedViolation(Duration duration) {
    ...
    final sustainedViolations =
        _violations.where((v) => v.timestamp.isAfter(cutoff)).toList();
...
class _PerformanceTrend {
  ...
  TrendInfo? calculateTrend() {
    ...
    final changeRate =
        slope * 60000; // Convert from per millisecond to per minute
