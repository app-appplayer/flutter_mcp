...
class DesktopBackgroundService implements BackgroundService {
  ...
  Future<void> initialize(BackgroundConfig? config) async {
    ...
    EventSystem.instance.publishTopic('background.initialized', {
      'platform': 'desktop',
  ...
  Future<bool> start() async {
    ...
    try {
      ...
      if (_isRunning) {
        ...
        // Publish start event
        EventSystem.instance.publishTopic('background.started', {
          'platform': 'desktop',
  ...
  Future<bool> stop() async {
    ...
    try {
      ...
      // Publish stop event
      EventSystem.instance.publishTopic('background.stopped', {
        'platform': 'desktop',
      ...
      _logger.info(
          'Desktop background service stopped after $_taskExecutionCount executions');
      return result ?? true;
    } catch (e, stackTrace) {
  ...
  Future<void> _performBackgroundTask() async {
    ...
    _currentTimerId =
        PerformanceMonitor.instance.startTimer('desktop.background.task');
    ...
    try {
      ...
      PerformanceMonitor.instance
          .incrementCounter('desktop.background.success');

      // Publish task completion event
      EventSystem.instance.publishTopic('background.task.completed', {
        'platform': 'desktop',
        'executionCount': _taskExecutionCount,
        'duration':
            DateTime.now().difference(_lastTaskExecution!).inMilliseconds,
      });
    } catch (e, stackTrace) {
      _errorCount++;
      _logger.severe(
          'Background task error ($_errorCount/$_maxConsecutiveErrors)',
          e,
          stackTrace);
      ...
      // Publish error event
      EventSystem.instance.publishTopic('background.task.error', {
        'platform': 'desktop',
        'error': e.toString(),
        'errorCount': _errorCount,
      });
      ...
      if (_errorCount >= _maxConsecutiveErrors) {
        _logger
            .severe('Too many consecutive errors, stopping background service');
        await stop();
      }
    }
  }
  ...
  Future<void> _performDefaultTasks() async {
    ...
    if (_config?.keepAlive ?? true) {
      EventSystem.instance.publishTopic('background.heartbeat', {
        'platform': 'desktop',
        'timestamp': DateTime.now().toIso8601String(),
        'uptime': DateTime.now()
            .difference(_lastTaskExecution ?? DateTime.now())
            .inSeconds,
      });
    }
  ...
  Future<void> scheduleTask(
    ...
    try {
      ...
      _logger
          .info('Scheduled task $taskId with delay ${delay.inSeconds} seconds');
    } catch (e, stackTrace) {
  ...
  Future<void> updateConfig(BackgroundConfig config) async {
    ...
    if (_isRunning && config.intervalMs != _config?.intervalMs) {
      _logger.info(
          'Restarting background service with new interval: ${config.intervalMs}ms');
      await stop();
      await start();
    }
  }
