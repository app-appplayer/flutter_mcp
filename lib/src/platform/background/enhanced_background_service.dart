...
abstract class EnhancedBackgroundService
    implements BackgroundService, HealthCheckProvider {
  final Logger logger;
    ...
    await EnhancedErrorHandler.instance.handleError(
      () async {
        _config = config is EnhancedBackgroundConfig
            ? config
            : EnhancedBackgroundConfig.fromBase(
                config ?? BackgroundConfig.defaultConfig());
  ...
  void _executeTask(String taskId) {
    ...
    EnhancedErrorHandler.instance
      ...
      logger.fine(
          'Task completed: ${task.name} (${task.executionTime?.inMilliseconds}ms)');
  ...
  Future<void> _waitForRunningTasks() async {
    ...
    logger
        .fine('Waiting for ${_runningTasks.length} running tasks to complete');
  ...
  Future<MCPHealthCheckResult> performHealthCheck() async {
    ...
    final failureRate =
        _totalTasksExecuted > 0 ? _totalTasksFailed / _totalTasksExecuted : 0.0;
    ...
    if (!_isRunning) {
      ...
      status = MCPHealthStatus.unhealthy;
      message =
          'High task failure rate: ${(failureRate * 100).toStringAsFixed(1)}%';
      ...
      status = MCPHealthStatus.degraded;
      message =
          'Elevated task failure rate: ${(failureRate * 100).toStringAsFixed(1)}%';
    } else {
  ...
  void _publishServiceEvent(String action) {
    _eventSystem.publishTopic('background_service.$action', {
  ...
  void _publishTaskEvent(String action, BackgroundTask task) {
    _eventSystem.publishTopic('background_task.$action', {
      'taskId': task.id,
      'taskName': task.name,
      'status': task.status.name,
      'priority': task.priority.name,
  ...
  void _publishJobEvent(String action, Job job) {
    _eventSystem.publishTopic('background_job.$action', {
      'jobName': job.name,
