...
/// Windows Tray manager implementation
class WindowsTrayManager implements TrayManager {
  ...
  Future<void> initialize(TrayConfig? config) async {
    _logger.debug('Windows tray manager initializing');
    ...
    try {
      ...
      _logger.debug('Windows tray manager initialized successfully');
    } catch (e, stackTrace) {
      _logger.error('Failed to initialize Windows tray manager', e, stackTrace);
  ...
  Future<void> setIcon(String path) async {
    _logger.debug('Setting Windows tray icon: $path');
  ...
  Future<void> setTooltip(String tooltip) async {
    _logger.debug('Setting Windows tray tooltip: $tooltip');
  ...
  Future<void> setContextMenu(List<TrayMenuItem> items) async {
    _logger.debug('Setting Windows tray context menu');
    ...
    try {
      ...
      _logger.error('Failed to set Windows tray context menu', e, stackTrace);
  ...
  Future<void> dispose() async {
    _logger.debug('Disposing Windows tray manager');
    ...
    try {
      ...
      _logger.error('Failed to dispose Windows tray manager', e, stackTrace);
...
/// Native tray listener implementation for Windows
class _WindowsTrayListener implements native_tray.TrayListener {
