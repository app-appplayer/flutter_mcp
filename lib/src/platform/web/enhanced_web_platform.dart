...
class EnhancedWebPlatform {
  ...
  final StreamController<String> _visibilityController = StreamController.broadcast();
  final StreamController<Map<String, dynamic>> _lifecycleController = StreamController.broadcast();
  ...
  static EnhancedWebPlatform get instance => _instance ??= EnhancedWebPlatform._internal();
  ...
  Future<void> _detectCapabilities() async {
    try {
      ...
      _supportsWebPush = _supportsServiceWorkers && 
          hasProperty(html.window, 'PushManager');
  ...
  Future<void> _initializeServiceWorker() async {
    ...
    try {
      ...
      _serviceWorkerRegistration = await html.window.navigator.serviceWorker!.register(
        '/flutter_mcp_sw.js',
        {'scope': '/'},
      );
  ...
  Future<void> _setupLifecycleHandling() async {
    try {
      ...
      if (hasProperty(html.document, 'onfreeze')) {
        callMethod(html.document, 'addEventListener', ['freeze', (event) {
          _lifecycleController.add({
            'type': 'freeze',
            'timestamp': DateTime.now().millisecondsSinceEpoch,
          });
      ...
      if (hasProperty(html.document, 'onresume')) {
        callMethod(html.document, 'addEventListener', ['resume', (event) {
          _lifecycleController.add({
            'type': 'resume',
            'timestamp': DateTime.now().millisecondsSinceEpoch,
          });
  ...
  Future<void> _checkStorageQuota() async {
    try {
      if (hasProperty(html.window.navigator, 'storage') && 
          hasProperty(getProperty(html.window.navigator, 'storage'), 'estimate')) {
  ...
  Future<String?> subscribeToWebPush({String? vapidPublicKey}) async {
    ...
    try {
      final pushManager = getProperty(_serviceWorkerRegistration!, 'pushManager');
  ...
  Future<bool> requestPersistentStorage() async {
    try {
      if (hasProperty(html.window.navigator, 'storage') && 
          hasProperty(getProperty(html.window.navigator, 'storage'), 'persist')) {
        final storage = getProperty(html.window.navigator, 'storage');
        final persistent = await promiseToFuture(callMethod(storage, 'persist', []));
  ...
  Stream<Map<String, dynamic>> get onLifecycleEvent => _lifecycleController.stream;
