...
class WebPlatformImprovements {
  ...
  static WebPlatformImprovements get instance =>
      _instance ??= WebPlatformImprovements._internal();
  ...
  Future<void> _detectCapabilities() async {
    try {
      ...
      _supportsStorageQuota = hasProperty(html.window, 'navigator') &&
          ...
          hasProperty(
              getProperty(getProperty(html.window, 'navigator'), 'storage'),
              'estimate');

      // Check performance observer API
      _supportsPerformanceObserver =
          hasProperty(html.window, 'PerformanceObserver');
  ...
  Future<void> _updateStorageInfo() async {
    try {
      ...
      final usagePercent =
          _storageQuota > 0 ? (_storageUsage / _storageQuota * 100) : 0;

      if (usagePercent > 80) {
        _logger.warning(
            'Storage usage high: ${usagePercent.toStringAsFixed(1)}% '
  ...
  Future<void> _initializePerformanceMonitoring() async {
    if (!_supportsPerformanceObserver) {
      _logger.info(
          'Performance Observer API not supported, using basic monitoring');
  ...
  void _setupPerformanceObserver(
      String entryType, Function(List<dynamic>) callback) {
    try {
      final observer = callMethod(html.window, 'PerformanceObserver', [
        (List<dynamic> entries) {
          callback(entries);
        }
      ]);

      callMethod(observer, 'observe', [
        {
      ...
      _logger
          .warning('Failed to setup performance observer for $entryType: $e');
    }
  }
  ...
  Future<void> _initializeConnectionHandling() async {
    ...
    try {
      ...
      _connectionType =
          getProperty(connection, 'effectiveType') as String? ?? 'unknown';
  ...
  Map<String, dynamic> getPlatformStatus() {
    return {
      ...
      'storage': {
        ...
        'usagePercent':
            _storageQuota > 0 ? (_storageUsage / _storageQuota * 100) : 0.0,
      },
      'visibility': {
  ...
  Stream<Map<String, dynamic>> get onConnectionChange =>
