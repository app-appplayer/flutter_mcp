...
class PluginCommunicationBus {
  static final PluginCommunicationBus _instance =
  ...
  void send(String channel, PluginMessage message) {
    _logger
        .fine('Sending message on channel $channel from ${message.senderId}');
    ...
    try {
      ...
      subscription = subscribe(channel, request.senderId).listen((message) {
        if (message.correlationId == request.id &&
            message.type == PluginMessageType.response) {
...
abstract class CommunicatingPlugin extends MCPPlugin {
  ...
  PluginCommunicationBus get communicationBus =>
      PluginCommunicationBus.instance;
  ...
  void subscribeToChannel(
      String channel, void Function(PluginMessage) handler) {
    final subscription =
        communicationBus.subscribe(channel, pluginId).listen(handler);
  ...
  /// Send a message
  void sendMessage(String channel, Map<String, dynamic> data,
      {PluginMessageType type = PluginMessageType.notification}) {
  ...
  /// Send a request and wait for response
  Future<PluginMessage> sendRequest(String channel, Map<String, dynamic> data,
    ...
    return await communicationBus.request(channel, request,
