...
class PluginLifecycleManager {
  ...
  /// Update plugin state
  Future<void> updatePluginState(
      String pluginName, PluginState newState) async {
    ...
    EventSystem.instance.publishTopic('plugin.state.changed', {
  ...
  Future<void> initializePlugin(
      String pluginName, MCPPlugin plugin, Map<String, dynamic> config) async {
  ...
  Future<void> startPlugin(String pluginName) async {
    ...
    if (state != PluginState.initialized && state != PluginState.stopped) {
      throw MCPPluginException(
          pluginName, 'Plugin must be initialized or stopped before starting');
    }
  ...
  Future<void> stopPlugin(String pluginName) async {
    ...
    if (state != PluginState.started) {
      throw MCPPluginException(
          pluginName, 'Plugin must be started before stopping');
    }
  ...
  Future<void> suspendPlugin(String pluginName) async {
    ...
    if (state != PluginState.started) {
      throw MCPPluginException(
          pluginName, 'Plugin must be started before suspending');
    }
  ...
  Future<void> resumePlugin(String pluginName) async {
    ...
    if (state != PluginState.suspended) {
      throw MCPPluginException(
          pluginName, 'Plugin must be suspended before resuming');
    }
  ...
  Future<void> _notifyStateListeners(
      String pluginName, PluginState from, PluginState to) async {
