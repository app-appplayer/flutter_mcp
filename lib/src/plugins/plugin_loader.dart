...
class DynamicPluginLoader {
  ...
  Future<void> loadPluginFromFile(String filePath,
      {Map<String, dynamic>? config}) async {
    if (kIsWeb) {
      throw MCPException(
          'Dynamic plugin loading from files is not supported on web platform');
    }
    ...
    try {
      ...
      throw MCPPluginException(
          filePath, 'Failed to load plugin from file', e, stackTrace);
    }
  }

  /// Load plugin from asset
  Future<void> loadPluginFromAsset(String assetPath,
      {Map<String, dynamic>? config}) async {
    ...
    try {
      ...
      throw MCPPluginException(
          assetPath, 'Failed to load plugin from asset', e, stackTrace);
    }
  }

  /// Load plugin from code string
  Future<void> _loadPluginCode(
      String source, String code, Map<String, dynamic>? config) async {
  ...
  Future<void> reloadPlugin(String pluginName) async {
    ...
    if (loadedPlugin.source.startsWith('assets/')) {
      await loadPluginFromAsset(loadedPlugin.source,
          config: loadedPlugin.config);
    } else {
      await loadPluginFromFile(loadedPlugin.source,
          config: loadedPlugin.config);
    }
