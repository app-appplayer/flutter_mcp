...
abstract class TypedPromptPlugin extends TypedMCPPlugin {
  ...
  Future<List<ValidationError>> validateArguments(Map<String, dynamic> arguments);
}
...
abstract class TypedNotificationPlugin extends TypedMCPPlugin {
  ...
  void onNotificationClicked(Function(String id, Map<String, dynamic> data) handler);
...
class TypedPluginRegistry {
  ...
  Future<PluginResult<String>> registerPlugin(
    ...
    try {
      ...
      if (_plugins[pluginType]!.containsKey(pluginName)) {
        return FailureResult<String>('Plugin with name "$pluginName" is already registered');
      }
      ...
      _publishLifecycleEvent(pluginName, PluginStatus.uninitialized, PluginStatus.initializing);
      ...
      _recordPluginMetric(pluginName, TimerMetric(
      ...
      if (initResult.isSuccess) {
        ...
        _publishLifecycleEvent(pluginName, PluginStatus.initializing, PluginStatus.active);
        
        _logger.info('Plugin $pluginName successfully registered and initialized');
        return SuccessResult<String>('Plugin registered successfully');
      } else {
        // Publish error event
        _publishErrorEvent(pluginName, 'Initialization failed', initResult.error);
  ...
  Future<PluginResult<String>> unregisterPlugin(String pluginName) async {
    ...
    try {
      // Publish lifecycle event
      _publishLifecycleEvent(pluginName, foundPlugin.status, PluginStatus.shuttingDown);
    ...
    try {
      ...
      if (validationErrors.isNotEmpty) {
        ...
        final response = ToolResponse(
          ...
          result: FailureResult('Validation failed: ${validationErrors.map((e) => e.toString()).join(', ')}'),
          executionTime: stopwatch.elapsed,
        );
        
        // Record metrics
        _recordPluginMetric(request.toolName, TimerMetric(
          name: 'tool.execution',
          duration: stopwatch.elapsed,
          operation: 'execute',
          success: false,
      ...
      stopwatch.stop();
      _logger.severe('Error executing typed tool ${request.toolName}', e, stackTrace);
      _publishErrorEvent(request.toolName, 'Tool execution failed', e.toString());
      ...
      return ToolResponse(
        toolName: request.toolName,
        result: FailureResult('Tool execution failed: ${e.toString()}', originalError: e),
        executionTime: stopwatch.elapsed,
      );
    }
  }
  ...
  Future<PluginResult<String>> shutdownAll() async {
    ...
    if (errors.isNotEmpty) {
      return FailureResult<String>('Errors occurred while shutting down plugins: $errors');
    }
  ...
  Future<PluginResult<String>> _validatePlugin(TypedMCPPlugin plugin, PluginConfig config) async {
    // Basic validation
    ...
    if (config.name != plugin.name) {
      return FailureResult<String>('Plugin name mismatch between plugin and config');
    }
  ...
  /// Publish lifecycle event
  void _publishLifecycleEvent(String pluginName, PluginStatus oldStatus, PluginStatus newStatus) {
