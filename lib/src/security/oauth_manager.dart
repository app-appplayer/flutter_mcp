...
class MCPOAuthManager {
  ...
  Future<String> authenticate(String llmId) async {
    ...
    if (cachedToken != null && cachedToken.refreshToken != null) {
      try {
        ...
        _logger.warning('Token refresh failed, will perform full authentication: $e');
      }
    }
  ...
  Future<String> _performAuthenticationFlow(String llmId) async {
    ...
    try {
      ...
      EventSystem.instance.publish('oauth.open_browser', {
        'llmId': llmId,
        'url': authUri.toString(),
      });
      
      // Publish event for apps to handle the callback
      EventSystem.instance.publish('oauth.authorization_requested', {
        'llmId': llmId,
        'state': state,
  ...
  Future<OAuthToken> _exchangeCodeForToken({
    ...
    try {
      ...
      request.headers.contentType = io.ContentType('application', 'x-www-form-urlencoded');
  ...
  Future<OAuthToken?> _loadStoredToken(String llmId) async {
    try {
      final dataJson = await _credentialManager.getCredential('oauth_token_$llmId');
      if (dataJson != null) {
  ...
  Future<void> revokeToken(String llmId) async {
    try {
      ...
      if (token != null && config != null && config.revokeUrl != null) {
        try {
          final request = await _httpClient.postUrl(Uri.parse(config.revokeUrl!));
          request.headers.contentType = io.ContentType('application', 'x-www-form-urlencoded');
