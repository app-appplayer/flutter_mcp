...
class SecurityAuditManager {
  ...
  bool checkAuthenticationAttempt(String userId, bool success,
      {Map<String, dynamic>? metadata}) {
  ...
  bool checkDataAccess(String userId, String resource, String action,
      {Map<String, dynamic>? metadata}) {
  ...
  Map<String, dynamic> generateSecurityReport() {
    ...
    final recent24h =
        _auditLog.where((e) => e.timestamp.isAfter(last24Hours)).toList();
    final recent7d =
        _auditLog.where((e) => e.timestamp.isAfter(last7Days)).toList();
    ...
    return {
      ...
      'failedLogins24h': recent24h
          .where(
              (e) => e.type == SecurityEventType.authentication && !e.success)
          .length,
      'suspiciousEvents24h':
          recent24h.where((e) => e.type == SecurityEventType.suspicious).length,
      'lockedOutUsers': _lockoutTimes.length,
      'activeSessions': _activeSessions.values
          .fold<int>(0, (sum, sessions) => sum + sessions.length),
      'highRiskUsers': _riskCache.entries
          .where((e) => e.value.score > _policy.riskThresholds['high']!)
          .length,
  ...
  void _checkSecurityViolations(SecurityAuditEvent event) {
    ...
    if (event.riskScore > _policy.riskThresholds['high']!) {
      _eventSystem.publishTopic(
          ...
          SecurityAlert(
            severity:
                _calculateSeverity(event), // Use proper severity calculation
            title: 'High Risk Security Event',
            message:
                'Event ${event.eventId} has high risk score: ${event.riskScore}',
            details: event.toJson(),
          ));
    }
  ...
  void _updateRiskAssessment(String userId, SecurityAuditEvent event) {
    ...
    final hoursSinceLastUpdate =
        DateTime.now().difference(existing.lastUpdated).inHours;
    final decayFactor = math.max(0.8, 1.0 - (hoursSinceLastUpdate * 0.01));

    final newScore = math.min(
        100, ((existing.score * decayFactor) + additionalRisk).round());
