...
class DefaultErrorRecoveryStrategy implements ErrorRecoveryStrategy {
  @override
  Future<T?> recover<T>(
      dynamic error, StackTrace stackTrace, String context) async {
    return null;
  }
...
class EnhancedErrorHandler {
  ...
  Future<T> _executeWithErrorHandling<T>(
    ...
    try {
      ...
      if (strategy != null && strategy.canRecover(error)) {
        ...
        if (recovered != null) {
          _logger.info(
              'Successfully recovered from error using $recoveryStrategy strategy');
          return recovered;
        }
      }
  ...
  void _updateErrorStatistics(String context, dynamic error) {
    ...
    if (_errorCounts[key]! > 10) {
      _logger.warning(
          'High error rate detected for $key: ${_errorCounts[key]} errors');
      _reportHealthIssue(
          context, 'High error rate: ${_errorCounts[key]} errors');
    }
  }
  ...
  void _handleMCPException(
    ...
    _eventSystem.publishTopic('error.mcp_exception', {
      'context': context,
  ...
  void _handleTimeoutException(
    ...
    _eventSystem.publishTopic('error.timeout', {
      'context': context,
      'component': component ?? 'unknown',
  ...
  void _handleGenericError(
    ...
    _eventSystem.publishTopic('error.generic', {
      'context': context,
      'component': component ?? 'unknown',
      'error': error.toString(),
      'errorType': error.runtimeType.toString(),
  ...
  void _registerDefaultHandlers() {
    ...
    registerErrorHandler<MCPNetworkException>(
        (error, stackTrace, context) async {
    ...
    registerErrorHandler<MCPValidationException>(
        (error, stackTrace, context) async {
    ...
    registerErrorHandler<MCPResourceNotFoundException>(
        (error, stackTrace, context) async {
