...
class EnhancedResourceCleanup {
  ...
  void registerResource<T extends Object>({
    ...
    if (_resources.containsKey(key)) {
      _logger.warning('Resource with key $key already exists, disposing previous resource');
    ...
    final disposableResource = EnhancedDisposableResource<T>(
      ...
      leakDetectionTimeout: leakDetectionTimeout ?? _defaultLeakDetectionTimeout,
    );
  ...
  Future<void> disposeResource(String key) async {
    await EnhancedErrorHandler.instance.handleError(
      () async {
        ...
        _logger.fine('Disposing resource: $key (type: ${resource.allocation.type})');
  ...
  Future<void> disposeAll() async {
    ...
    _logger.info('Resource cleanup complete. Disposed: $_totalResourcesDisposed, Failed: $_failedDisposals');
  }
  ...
  void _publishResourceEvent(String action, String resourceKey, String resourceType) {
    _eventSystem.publish('resource.$action', {
      'key': resourceKey,
      'type': resourceType,
  ...
  void _publishLeakDetectionEvent(List<String> leakedResources) {
    _eventSystem.publish('resource.leak_detected', {
  ...
  List<Map<String, dynamic>> getResourceDetails() {
    return _resources.values.map((resource) => {
      'key': resource.key,
    ...
    for (final resourceKey in _managedResources.reversed) {
      try {
        ...
        Logger(runtimeType.toString()).severe(
          'Failed to dispose managed resource: $resourceKey', e
        );
      }
    }
