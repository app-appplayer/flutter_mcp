...
class ErrorMonitor {
  ...
  void _initializeConfiguration() {
    try {
      ...
      _monitoringInterval = config.getDuration('interval', defaultValue: Duration(minutes: 1));
  ...
  void startMonitoring() {
    ...
    _logger.info('Started error monitoring with interval: ${_monitoringInterval.inMinutes} minutes');
  }
  ...
  void _collectMetrics() {
    ...
    final recentErrors = MCPErrorHandler.instance.getRecentErrors(within: _monitoringInterval);
  ...
  ErrorSeverity _getErrorSeverity(dynamic error) {
    if (error.toString().contains('Security') || error.toString().contains('Initialization')) {
      return ErrorSeverity.critical;
    }
    if (error.toString().contains('Platform') || error.toString().contains('Network')) {
      return ErrorSeverity.high;
    }
    if (error.toString().contains('Validation') || error.toString().contains('Timeout')) {
  ...
  void _checkForAlerts(ErrorReport report) {
    ...
    final recentErrors = MCPErrorHandler.instance.getRecentErrors(within: Duration(minutes: 10));
  ...
  void _createAlert(
    ...
    if (existingAlert != null) {
      ...
      // Publish alert event
      EventSystem.instance.publish('error.alert', {
        'alertId': alert.id,
  ...
  void _updateDashboard() {
    ...
    final dashboard = ErrorDashboard(
      ...
      totalErrors: _metrics.fold<int>(0, (sum, metric) => sum + metric.totalErrors),
  ...
  double _calculateErrorRate() {
    ...
    final totalErrors = recentMetrics.fold<int>(0, (sum, metric) => sum + metric.totalErrors);
  ...
  int _calculateSystemHealth() {
    ...
    final unrecoveredErrors = recentMetrics.fold<int>(0, (sum, metric) => 
        sum + (metric.totalErrors - metric.recoveredErrors));
  ...
  /// Get alerts stream
  Stream<ErrorAlert> get alertsStream => _alertController?.stream ?? Stream.empty();
  
  /// Get dashboard stream
  Stream<ErrorDashboard> get dashboardStream => _dashboardController?.stream ?? Stream.empty();
  ...
  Map<String, dynamic> exportData() {
    return {
      ...
      'patterns': _patterns.map((key, pattern) => MapEntry(key, pattern.toJson())),
