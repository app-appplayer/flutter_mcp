...
class RealtimePerformanceMonitor {
  static final RealtimePerformanceMonitor _instance = RealtimePerformanceMonitor._internal();
  ...
  final _metricsStreamController = StreamController<Map<String, dynamic>>.broadcast();
  final _alertsStreamController = StreamController<PerformanceAlert>.broadcast();
  ...
  void _updateMetrics() {
    try {
      ...
      EventSystem.instance.publish('performance.metrics.updated', realtimeMetrics);
  ...
  /// Calculate additional real-time metrics
  Map<String, dynamic> _calculateRealtimeMetrics(Map<String, dynamic> baseMetrics) {
  ...
  Map<String, dynamic> _calculateTrends(MetricSnapshot recent, MetricSnapshot previous) {
    final trends = <String, dynamic>{};
    
    // Calculate memory trend
    final recentMemory = recent.metrics['resources']?['memory.usageMB']?['current'] ?? 0;
    final previousMemory = previous.metrics['resources']?['memory.usageMB']?['current'] ?? 0;
  ...
  void _checkThresholds(Map<String, dynamic> metrics) {
    // Check memory usage
    final memoryUsage = metrics['resources']?['memory.usageMB']?['current'] ?? 0;
    ...
    if (memoryUsage > memoryThreshold) {
      _emitAlert(PerformanceAlert(
        ...
        message: 'Memory usage exceeded threshold: ${memoryUsage}MB > ${memoryThreshold}MB',
        value: memoryUsage,
    ...
    if (errorRate > errorThreshold) {
      _emitAlert(PerformanceAlert(
        ...
        message: 'Error rate exceeded threshold: $errorRate% > $errorThreshold%',
        value: errorRate,
  ...
  void _emitAlert(PerformanceAlert alert) {
    ...
    // Publish alert event
    EventSystem.instance.publish('performance.alert', {
      'alert': alert.toJson(),
