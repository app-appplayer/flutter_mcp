...
class WebMemoryMonitor {
  ...
  void startMonitoring({Duration? interval}) {
    ...
    _logger.info('Started web memory monitoring with interval: ${monitoringInterval.inMilliseconds}ms');
  }
  ...
  Future<void> _collectMemorySnapshot() async {
    ...
    try {
      ...
      _logger.fine('Memory snapshot: ${snapshot.usedJSHeapSize}MB used, ${snapshot.totalJSHeapSize}MB total');
    } catch (e) {
  ...
  Future<WebMemorySnapshot> _getMemoryInfo() async {
    ...
    try {
      ...
      _logger.fine('Performance Observer API not available, using estimation: $e');
    }
  ...
  Future<WebMemorySnapshot> _getPerformanceObserverMemory() async {
    try {
      ...
      final memoryEstimate = _calculateMemoryFromPerformance(performanceEntries);
  ...
  int _getBaseMemoryEstimate() {
    final appRuntime = DateTime.now().difference(_snapshots.isNotEmpty ? 
      _snapshots.first.timestamp : DateTime.now()).inMinutes;
    
    // Base memory usage increases over time
    return 60 + (appRuntime * 2).clamp(0, 100); // 60MB base + 2MB per minute, max 160MB
  }
  ...
  int _getMemoryVariance() {
    ...
    final recentReadings = _snapshots.length > 5 
        ? _snapshots.sublist(_snapshots.length - 5).map((s) => s.usedJSHeapSize).toList()
        : _snapshots.map((s) => s.usedJSHeapSize).toList();
    final average = recentReadings.reduce((a, b) => a + b) / recentReadings.length;
    final variance = recentReadings.map((r) => (r - average).abs()).reduce((a, b) => a + b) / recentReadings.length;
  ...
  Map<String, int> _calculateMemoryFromPerformance(Map<String, dynamic> perfData) {
    ...
    final timingMemory = (navigationTiming / 100).round(); // Memory based on load time
  ...
  Map<String, int> _estimateMemoryFromSystemInfo(Map<String, dynamic> systemInfo) {
  ...
  WebMemorySnapshot _estimateMemoryUsage(DateTime timestamp) {
    try {
      ...
      final appStartTime = timestamp.subtract(Duration(milliseconds: DateTime.now().millisecondsSinceEpoch - (_snapshots.isEmpty ? 0 : _snapshots.first.timestamp.millisecondsSinceEpoch)));
      final runtimeMinutes = DateTime.now().difference(appStartTime).inMinutes.toDouble();
      ...
      var estimatedUsedMB = baseMemoryMB + timeBasedMemoryMB + activityMemoryMB - gcReduction;
      estimatedUsedMB = math.max(50, estimatedUsedMB); // Minimum 50MB
      
      final estimatedTotalMB = (estimatedUsedMB * 1.4).round(); // Add 40% headroom
  ...
  Map<String, dynamic> getStatistics() {
    ...
    final peak = _snapshots.map((s) => s.usedJSHeapSize).reduce((a, b) => a > b ? a : b);
    final average = _snapshots.map((s) => s.usedJSHeapSize).reduce((a, b) => a + b) / _snapshots.length;
  ...
  void suggestGarbageCollection() {
    try {
      ...
      final temp = List.generate(1000, (i) => {
