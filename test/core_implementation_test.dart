  ...
  group('Core Implementation Tests', () {
    ...
    group('Credential Manager Tests', () {
      test('should store and retrieve credentials', () async {
        ...
        final credentialManager =
            await CredentialManager.initialize(secureStorage);
        ...
        final retrievedValue =
            await credentialManager.getCredential('test_key');
        ...
        final existsBeforeDelete =
            await credentialManager.hasCredential('test_key');
      ...
      test('should handle credential metadata', () async {
        ...
        final result =
            await credentialManager.getCredentialWithMetadata('meta_key');
    ...
    group('Event System Tests', () {
      test('should publish and subscribe to events', () async {
        ...
        // Subscribe to events
        final subscription =
            await eventSystem.subscribeTopic('test_event', (data) {
          receivedEvent = 'test_event';
          receivedData = data as Map<String, dynamic>?;
        });

        // Publish event
        eventSystem.publishTopic('test_event', {'message': 'hello'});
      ...
      test('should handle multiple subscribers', () async {
        ...
        final sub1 = await eventSystem.subscribeTopic('multi_event', (data) {
          subscriber1CallCount++;
        });

        final sub2 = await eventSystem.subscribeTopic('multi_event', (data) {
          subscriber2CallCount++;
        });

        // Publish event
        eventSystem.publishTopic('multi_event', {});
        ...
        // Cleanup
        await eventSystem.unsubscribe(sub1);
        await eventSystem.unsubscribe(sub2);
      });
    });
    ...
    group('Circuit Breaker Tests', () {
      test('should prevent calls when circuit is open', () async {
        ...
        expect(
          () async =>
              await circuitBreaker.execute(() async => 'should_not_execute'),
