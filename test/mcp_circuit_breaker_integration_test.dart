...
class CircuitBreakerTestFlutterMCP extends TestFlutterMCP {
  ...
  CircuitBreakerTestFlutterMCP(super.platformServices) {
    ...
    EventSystem.instance.subscribeTopic('circuit_breaker.opened', (data) {
      print('Received circuit_breaker.opened event: $data');
      circuitBreakerEvents.add({'type': 'opened', 'data': data});
    });

    EventSystem.instance.subscribeTopic('circuit_breaker.closed', (data) {
      print('Received circuit_breaker.closed event: $data');
      circuitBreakerEvents.add({'type': 'closed', 'data': data});
    });
  }
  ...
  void _initializeCircuitBreakers() {
    ...
    _circuitBreakers['llm.chat'] = CircuitBreaker(
      ...
      failureThreshold: 3,
      resetTimeout:
          Duration(milliseconds: 200), // Set short timeout for testing
      onOpen: () async {
  ...
  Future<T> executeWithCircuitBreaker<T>(
      String breakerName, Future<T> Function() operation) async {
  ...
  Future<void> recordCircuitBreakerFailure(
      String breakerName, dynamic error) async {
    final breaker = _circuitBreakers[breakerName];
    if (breaker == null) {
      throw MCPException('Circuit breaker not found: $breakerName');
    }
    ...
    print(
        'Recording failure for $breakerName. Current state: ${breaker.state}');
  ...
  Future<void> resetCircuitBreaker(String breakerName) async {
    final breaker = _circuitBreakers[breakerName];
    if (breaker == null) {
      throw MCPException('Circuit breaker not found: $breakerName');
    }
  ...
  Future<void> resetAllCircuitBreakers() async {
    for (final entry in _circuitBreakers.entries) {
      await entry.value.reset();
    }
  }
}
...
void main() {
  ...
  setUp(() async {
    ...
    // Reset EventSystem to avoid cross-test contamination
  ...
  group('Circuit Breaker Integration Tests', () {
    test('Circuit breaker opens after multiple failures', () async {
      ...
      await flutterMcp.resetAllCircuitBreakers();

      // Verify initial state is closed
      expect(flutterMcp.getCircuitBreakerState('llm.chat'),
          CircuitBreakerState.closed);

      // Record failures to reach threshold
      for (int i = 0; i < 3; i++) {
        await flutterMcp.recordCircuitBreakerFailure(
            'llm.chat', 'Test failure $i');
      }
      ...
      expect(
          flutterMcp.circuitBreakerEvents
              .where((e) => e['type'] == 'opened')
              .length,
          1);

      // Verify operation fails when circuit is open
      expect(
        () => flutterMcp.executeWithCircuitBreaker(
            'llm.chat', () async => 'result'),
    ...
    test('Circuit breaker transitions from open to half-open to closed',
      ...
      await Future.delayed(
          Duration(milliseconds: 300)); // 1.5x the reset timeout for stability

      // Wait for event processing
      await Future.delayed(Duration(milliseconds: 10));

      // Execute a successful operation
      final result = await flutterMcp.executeWithCircuitBreaker(
          'llm.chat', () async => 'success');
    ...
    test('Circuit breaker prevents cascading failures', () async {
      ...
      final result1 = await flutterMcp.executeWithCircuitBreaker(
          'tool.call', flakyOperation);
      ...
      print(
          'After second execution - State: ${flutterMcp.getCircuitBreakerState('tool.call')}');
      print(
          'Failure count: ${flutterMcp._circuitBreakers['tool.call']!.failureCount}');
      ...
      try {
        ...
        print(
            'After third execution - State: ${flutterMcp.getCircuitBreakerState('tool.call')}');
        print(
            'Failure count: ${flutterMcp._circuitBreakers['tool.call']!.failureCount}');
      }
      ...
      expect(flutterMcp.getCircuitBreakerState('tool.call'),
          CircuitBreakerState.open);
      ...
      final result2 = await flutterMcp.executeWithCircuitBreaker(
          'tool.call', flakyOperation);
    ...
    test('Multiple circuit breakers operate independently', () async {
      ...
      final result = await flutterMcp.executeWithCircuitBreaker(
          'llm.chat', () async => 'llm success');
      expect(result, 'llm success');

      // Tool operations should fail
      expect(
        () => flutterMcp.executeWithCircuitBreaker(
            'tool.call', () async => 'tool success'),
        throwsA(isA<CircuitBreakerOpenException>()),
      );

      // Reset the tool circuit breaker
      await flutterMcp.resetCircuitBreaker('tool.call');
      expect(flutterMcp.getCircuitBreakerState('tool.call'),
          CircuitBreakerState.closed);

      // Tool operations should now succeed
      final toolResult = await flutterMcp.executeWithCircuitBreaker(
          'tool.call', () async => 'tool success');
    ...
    test('Recovery after transient failures', () async {
      ...
      for (int i = 0; i < 3; i++) {
        try {
          await flutterMcp.executeWithCircuitBreaker(
              'llm.chat', transientService);
          fail('Should have thrown an exception');
        } catch (e) {
          // Exception is expected
          print(
              'Failure $i recorded. Current state: ${flutterMcp.getCircuitBreakerState('llm.chat')}');
        }
      }
      ...
      print(
          'Circuit state after timeout: ${flutterMcp.getCircuitBreakerState('llm.chat')}');
