...
class ConcurrentOperationsFlutterMCP extends TestFlutterMCP {
  ...
  Future<String> performConcurrentTask(
    String taskId, {
    ...
    if (trackResource && useResourceManager) {
      ...
      resourceManager.register<String>('task_$taskId', taskId, (value) async {
  ...
  Future<List<String>> executeParallelOperations(
    int count, {
    ...
    for (int i = 0; i < count; i++) {
      final delay =
          Duration(milliseconds: random.nextInt(maxDelay.inMilliseconds));
  ...
  Future<String> performPooledOperation(
      ObjectPool<Stopwatch> pool, String taskId) async {
    ...
    try {
      ...
      pooledObject.stop();
      operationLog.add(
          'Task $taskId used object for ${pooledObject.elapsedMilliseconds}ms');
...
void main() {
  ...
  group('Concurrent Operations Tests', () {
    test('Execute multiple operations in parallel', () async {
      ...
      expect(
          flutterMcp.operationLog
              .where((log) => log.startsWith('Started task'))
              .length,
          20);
      expect(
          flutterMcp.operationLog
              .where((log) => log.startsWith('Completed task'))
              .length,
          20);
      ...
      expect(
          flutterMcp.operationLog
              .where((log) => log.startsWith('Disposed task'))
              .length,
          20);
    });
    ...
    test('Object pool handles concurrent requests', () async {
      ...
      final futures = List.generate(10,
          (i) => flutterMcp.performPooledOperation(stopwatchPool, 'pool_$i'));
      ...
      expect(
          flutterMcp.operationLog
              .where((log) => log.contains('released object'))
              .length,
          10);
    ...
    test('Controlled concurrency with semaphore', () async {
      ...
      expect(
          results.every((result) => result.startsWith('Result for')), isTrue);
    ...
    test('Resource cleanup after concurrent operations', () async {
      ...
      for (int i = 0; i < 5; i++) {
        flutterMcp.resourceManager
            .addDependency('task_op_$i', 'parent_resource');
      }
      ...
      final logs = flutterMcp.operationLog
          .where((log) => log.startsWith('Disposed'))
          .toList();
      print('Disposal logs: $logs');

      final parentIndex =
          logs.indexWhere((log) => log.contains('parent resource'));
      ...
      for (int i = 0; i < 5; i++) {
        final dependentIndex =
            logs.indexWhere((log) => log.contains('task op_$i'));
        print('Dependent task_op_$i disposed at index: $dependentIndex');
        if (dependentIndex != -1 &&
            parentIndex != -1 &&
            dependentIndex < parentIndex) {
      ...
      expect(atLeastOneDependentBeforeParent, isTrue,
          reason:
              'Dependent resources should be disposed before parent resource');
