...
class LifecycleTestFlutterMCP extends TestFlutterMCP {
  ...
  Future<void> setLifecycleChangeListener(Function(AppLifecycleStateTest) listener) async {
    // Override to use our mock lifecycle listener
...
void main() {
  ...
  setUp(() {
    ...
    when(mockPlatformServices.setLifecycleChangeListener(any)).thenAnswer((_) async {});
    
    flutterMcp = LifecycleTestFlutterMCP(mockPlatformServices, mockLifecycleListener);
  });
  ...
  group('Lifecycle Management Tests', () {
    test('App responds to lifecycle state changes', () async {
      ...
      expect(flutterMcp.lifecycleEvents.contains(AppLifecycleStateTest.paused.toString()), isTrue);
      expect(flutterMcp.resourceOperations.contains('paused_resources_saved'), isTrue);
      ...
      expect(flutterMcp.lifecycleEvents.contains(AppLifecycleStateTest.resumed.toString()), isTrue);
      expect(flutterMcp.resourceOperations.contains('resumed_resources_restored'), isTrue);
    ...
    test('App cleanup on detached state', () async {
      ...
      mockLifecycleListener.simulateLifecycleChange(AppLifecycleStateTest.detached);
      
      // Verify app responded to detached state
      expect(flutterMcp.lifecycleEvents.contains(AppLifecycleStateTest.detached.toString()), isTrue);
      expect(flutterMcp.resourceOperations.contains('detached_resources_cleaned'), isTrue);
    ...
    test('Background service state maintained during lifecycle changes', () async {
    ...
    test('Multiple rapid lifecycle transitions handled properly', () async {
      ...
      mockLifecycleListener.simulateLifecycleChange(AppLifecycleStateTest.inactive);
      mockLifecycleListener.simulateLifecycleChange(AppLifecycleStateTest.paused);
      mockLifecycleListener.simulateLifecycleChange(AppLifecycleStateTest.inactive);
      mockLifecycleListener.simulateLifecycleChange(AppLifecycleStateTest.resumed);
      mockLifecycleListener.simulateLifecycleChange(AppLifecycleStateTest.inactive);
      ...
      expect(flutterMcp.resourceOperations.where((op) => op == 'inactive_resources_minimized').length, 3);
      expect(flutterMcp.resourceOperations.where((op) => op == 'paused_resources_saved').length, 2);
      expect(flutterMcp.resourceOperations.where((op) => op == 'resumed_resources_restored').length, 1);
