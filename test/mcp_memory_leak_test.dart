...
void main() {
  ...
  group('Memory Leak Tests', () {
    test('No resource leaks after initialization and shutdown', () async {
      ...
      expect(flutterMcp.getResourceCount(), 0, reason: 'All resources should be disposed after shutdown');
      
      // Verify all priority resources were disposed
      final highPrioIndex = flutterMcp.disposedResources.indexOf('highPrioResource');
      final mediumPrioIndex = flutterMcp.disposedResources.indexOf('mediumPrioResource');
      final lowPrioIndex = flutterMcp.disposedResources.indexOf('lowPrioResource');
      
      expect(highPrioIndex >= 0, isTrue, reason: 'High priority resource should be disposed');
      expect(mediumPrioIndex >= 0, isTrue, reason: 'Medium priority resource should be disposed');
      expect(lowPrioIndex >= 0, isTrue, reason: 'Low priority resource should be disposed');
      ...
      expect(highPrioIndex < mediumPrioIndex, isTrue, reason: 'High priority should be disposed before medium priority');
      expect(mediumPrioIndex < lowPrioIndex, isTrue, reason: 'Medium priority should be disposed before low priority');
    });
    
    test('Resources are disposed after multiple init-shutdown cycles', () async {
      final config = MCPConfig(
        appName: 'Cycle Test',
        appVersion: '1.0.0',
        autoStart: false,
      ...
      for (int i = 0; i < 3; i++) {
        ...
        // Verify cleanup
        expect(flutterMcp.getResourceCount(), 0, reason: 'Resources should be completely disposed after shutdown');
        expect(flutterMcp.disposedResources.contains('resource_$i'), isTrue, reason: 'Specific resource should be in the disposed list');
      }
    });
    ...
    test('Resource dependencies are properly disposed', () async {
      ...
      expect(flutterMcp.resourceManager.hasResource('parent'), isFalse, reason: 'Parent should be disposed');
      expect(flutterMcp.resourceManager.hasResource('child1'), isFalse, reason: 'Child1 should be disposed because it depends on parent');
      expect(flutterMcp.resourceManager.hasResource('child2'), isFalse, reason: 'Child2 should be disposed because it depends on parent');
      ...
      expect(childIndex1 < parentIndex, isTrue, reason: 'Child1 should be disposed before parent');
      expect(childIndex2 < parentIndex, isTrue, reason: 'Child2 should be disposed before parent');
  ...
  group('High Memory Situation Tests', () {
    ...
    test('Resource manager can handle large number of resources', () async {
      ...
      expect(stopwatch.elapsedMilliseconds, lessThan(10000), 
          reason: 'Resource disposal should complete within reasonable time even with many resources');
    });
  });
