  ...
  group('MCPScheduler Tests', () {
    ...
    group('Job Execution', () {
      ...
      test('should not execute job concurrently', () {
        ...
        scheduler.checkJobsNow();
        scheduler
            .checkJobsNow(); // This should be skipped due to the job already running
    ...
    group('Job Dependencies and Ordering', () {
      test('should handle multiple jobs with different intervals', () async {
        ...
        final fastJob = MCPJob(
          name: 'fast_job',
          interval: Duration(
              milliseconds: 500), // Faster than 1 second check interval
          task: () async {
            fastJobCount++;
        ...
        expect(slowJobCount,
            greaterThanOrEqualTo(1)); // At least one execution for slow job
      });
    ...
    group('Job History and Monitoring', () {
      test('should track job execution history', () async {
        ...
        expect(
            scheduler.activeJobCount, equals(0)); // No jobs should be running
      });
    });
    ...
    group('Resource Management', () {
      ...
      test('should handle scheduler restart', () async {
        ...
        expect(countAfterFirstRun,
            greaterThan(0)); // Should have executed at least once
    ...
    group('Edge Cases and Error Scenarios', () {
      test('should handle removing non-existent job gracefully', () {
        expect(() => scheduler.removeJob('non_existent_job'),
      ...
      test('should handle jobs with very short intervals', () async {
        ...
        expect(executionCount,
            lessThanOrEqualTo(3)); // Should not execute too many times
      }, timeout: Timeout(Duration(seconds: 5)));
      ...
      test('should handle many jobs efficiently', () async {
        ...
        for (int i = 0; i < jobCount; i++) {
          final job = MCPJob(
            name: 'load_job_\$i',
            interval:
                Duration(milliseconds: 300 + (i * 50)), // Reasonable intervals
            task: () async {
              totalExecutions++;
