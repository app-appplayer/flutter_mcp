...
void main() {
  group('Memory Management Analysis Tests', () {
    test('Actual ProcessInfo Memory Tracking', () async {
      ...
      print(
          'After memory allocation: ${(afterAllocationMemory / 1024 / 1024).toStringAsFixed(2)} MB');
      print(
          'Memory increase: ${(memoryIncrease / 1024 / 1024).toStringAsFixed(2)} MB');

      // Verify that memory actually increased
      expect(memoryIncrease,
          greaterThan(5 * 1024 * 1024)); // At least 5MB increase
      ...
      for (int i = 0; i < 3; i++) {
        // Create and immediately discard temporary objects to trigger GC
        List.filled(100000, 0); // Temporary list to trigger GC
      ...
      expect(
          afterCleanupMemory,
          lessThan(afterAllocationMemory +
              50 * 1024 * 1024)); // Allow up to 50MB additional increase
    });
    ...
    test('MemoryManager Standalone Function Test', () async {
      ...
      print(
          'Current memory: ${currentMemoryMB}MB, Test threshold: ${lowThreshold}MB');
      ...
      resourceManager.register<String>(
        ...
        (resource) async {
          await Future.delayed(
              Duration(milliseconds: 50)); // Simulate cleanup time
          disposedOrder.add('A');
    ...
    test('Memory Chunk Processing Efficiency', () async {
      ...
      final largeDataSet = List.generate(
          1000,
          (index) =>
              'item_$index' * 100); // Each item is approximately 500 bytes
      ...
      print(
          'Memory after normal processing: ${(afterNormalMemory / 1024 / 1024).toStringAsFixed(2)} MB');
      ...
      print(
          'Memory after chunk processing: ${(afterChunkMemory / 1024 / 1024).toStringAsFixed(2)} MB');
      ...
      print(
          'Normal processing memory increase: ${(normalMemoryIncrease / 1024 / 1024).toStringAsFixed(2)} MB');
      print(
          'Chunk processing memory increase: ${(chunkMemoryIncrease / 1024 / 1024).toStringAsFixed(2)} MB');

      // Chunk processing should show controlled memory usage
      expect(chunkMemoryIncrease,
    ...
    test('Parallel Processing Concurrency Control', () async {
      ...
      print(
          'Concurrency tracking: ${concurrentTracker.take(10).toList()}...'); // Print only first 10
    ...
    test('EventSystem Memory Event Propagation (Type Safe)', () async {
      ...
      // Reset event system before test
      ...
      final legacyToken =
          await eventSystem.subscribeTopic('memory.high', (data) {
        receivedLegacyEvents.add(data as Map<String, dynamic>);
        print('Legacy memory event received: ${(data as Map)['currentMB']}MB');
      });
      ...
      // Also publish to topic for backward compatibility test
