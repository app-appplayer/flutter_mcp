  ...
  group('Typed Event System Tests', () {
    ...
    test('should publish and receive typed server events', () async {
      ...
      final token = typedEventSystem.subscribe<ServerEvent>((event) {
        receivedEvents.add(event);
        print('Received typed server event: ${event.serverId} - ${event.status.name}');
      });
    ...
    test('should work with EventSystem integration', () async {
      ...
      final legacyToken = eventSystem.subscribe<Map<String, dynamic>>('memory.high', (event) {
        legacyEvents.add(event);
        print('Received via legacy subscription: ${event['currentMB']}MB');
      });
      ...
      await typedEventSystem.unsubscribe(typedToken);
