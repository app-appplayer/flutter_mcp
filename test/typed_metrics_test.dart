...
void main() {
  group('Typed Performance Metrics Tests', () {
    ...
    test('should create and analyze MetricCollection', () {
      ...
      expect(
          stats['max'], equals(100000.0)); // TimerMetric value in microseconds
    ...
    test('should handle event publishing for typed metrics', () async {
      ...
      EventSystem.instance.subscribe<PerformanceEvent>((event) {
        // Only capture events from our test metrics
        if (event.metricName == 'isolated.cpu.usage' ||
            event.metricName == 'isolated.errors.count') {
      ...
      final cpuEvent = receivedEvents
          .firstWhere((e) => e.metricName == 'isolated.cpu.usage');
      expect(cpuEvent.value, equals(75.0));
      expect(cpuEvent.capacity, equals(100.0));
      expect(cpuEvent.type, equals(MetricType.gauge));

      final errorEvent = receivedEvents
          .firstWhere((e) => e.metricName == 'isolated.errors.count');
    ...
    test('should handle metric collection serialization', () {
      ...
      expect(restoredCollection.metrics.length,
          equals(originalCollection.metrics.length));
      ...
      final resources =
          restoredCollection.getMetricsByType<ResourceUsageMetric>();
      expect(resources.length, equals(1));
